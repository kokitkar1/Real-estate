            ...
            <div>
                {/* <Swiper navigation>
                    {listing.imageUrls.map((url) => (
                        <SwiperSlide key={url}>
                            <div className='h-[850px]' style={{background: `url(${url}) center no-repeat`, backgroundSize: 'cover'}}>
                        ...
                        <SwiperSlide key={url}>
                            <div className='relative'>
                                <div className='w-full' style={{ paddingBottom: '75%' }} >
                                    <div className='absolute inset-0 bg-center bg-no-repeat bg-cover' style={{ backgroundImage: `url(${url})` }}/>
                                </div>
                            </div>
                        </SwiperSlide>
                    ))}
                ...
                <Swiper className="custom-swiper" navigation>
                    {listing.imageUrls.map((url) => (
                        <SwiperSlide key={url}>
                            <div className='relative'>
                                <div className='w-full ' style={{ paddingBottom: '55%' }} >
                                    <div className='absolute inset-0 bg-center bg-no-repeat bg-cover' style={{ backgroundImage: `url(${url})` }}/>
                                </div>
                                <div className='absolute top-0 right-0 mt-2 mr-2 z-10 border rounded-full w-12 h-12 flex justify-center items-center bg-slate-100 cursor-pointer'>
                                    <FaShare className='text-slate-500' onClick={() => { navigator.clipboard.writeText(window.location.href); setCopied(true); setTimeout(() => { setCopied(false); }, 2000); }} />
                                </div>
                            </div>
                        </SwiperSlide>
                ...
                {copied && ( <p className='fixed top-[23%] right-[5%] sm:top-[20%] sm:right-[5%] md:right-[7%] z-10 rounded-md bg-slate-100 p-2'>Link copied!</p>)}
